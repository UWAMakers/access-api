...
module.exports = (options = {}) => {
  return async (context) => {
    ...
    if (!training)
      training = await app.service('trainings').get(record.trainingId);
    ...
    const incomplete = items.reduce((a, item) => {
      ...
      if (
        item.expiry &&
        moment(compItem.confirmedAt).add(item.expiry, 'weeks').valueOf() <
          Date.now()
      )
        return [...a, compItem];
      if (item.type === 'quiz' && compItem.score < (item.requiredScore || 0))
        return [...a, compItem];
      if (item.type === 'completion' && compItem.status !== 'complete')
        return [...a, compItem];
      return a;
    }, []);
